<div>
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
            <a href="{{ route('contacts.index') }}" wire:navigate class="text-sm text-blue-600 hover:underline">← Kembali ke daftar</a>
            <h1 class="mt-1 text-2xl font-semibold">{{ $contact->first_name }} {{ $contact->last_name }}</h1>
            <p class="mt-1 text-sm text-zinc-600 dark:text-zinc-400">{{ $contact->email }}</p>
        </div>

        <div class="flex flex-wrap gap-2">
            <livewire:shared.log-activity-modal :morph-type="\App\Models\Contact::class" :morph-id="$contact->id" :key="'log-act-'.$contact->id" />
            <button type="button"
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div class="lg:col-span-2 space-y-6">
            <div class="border-b border-zinc-200 dark:border-zinc-800">
                <nav class="-mb-px flex gap-4" aria-label="Tabs">
                    @foreach (['overview' => 'Overview', 'activities' => 'Aktivitas', 'notes' => 'Notes', 'deals' => 'Related Deals'] as $key => $label)
                        <button type="button"
            ...
            @if ($activeTab === 'overview')
                <x-card title="Detail">
                    <dl class="grid grid-cols-1 gap-x-4 gap-y-3 text-sm sm:grid-cols-2">
                        ...
                        <div>
                            <dt class="text-xs font-medium uppercase tracking-wide text-zinc-500">Mobile</dt>
                            <dd>{{ $contact->mobile ?? '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-medium uppercase tracking-wide text-zinc-500">Job Title</dt>
                            <dd>{{ $contact->job_title ?? '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-medium uppercase tracking-wide text-zinc-500">Source</dt>
                            <dd>{{ $contact->source ? ucfirst($contact->source) : '—' }}</dd>
                        </div>
                        <div>
                    ...
                    @if ($contact->notes)
                        <div class="mt-4">
                            ...
                            <dd class="mt-1 whitespace-pre-line text-sm">{{ $contact->notes }}</dd>
                        </div>
                    @endif
                </x-card>
            ...
            @elseif ($activeTab === 'notes')
                <livewire:shared.note-editor :morph-type="\App\Models\Contact::class" :morph-id="$contact->id" :key="'note-'.$contact->id" />
            @elseif ($activeTab === 'deals')
                <x-card>
                    @if ($this->deals->isEmpty())
                        <x-empty-state title="Belum ada deals" description="Deal yang terkait dengan contact ini akan muncul di sini." />
                    @else
        ...
        <aside class="space-y-6">
            <x-card title="Owner">
                <livewire:shared.owner-badge :user-id="$contact->owner_id" :key="'owner-'.$contact->id" />
            </x-card>

            <x-card title="Tags">
                @can('update', $contact)
                    <livewire:shared.tag-selector :morph-type="\App\Models\Contact::class" :morph-id="$contact->id" :selected="$contact->tags->pluck('id')->all()" :key="'tags-'.$contact->id" />
                @else
                    <div class="flex flex-wrap gap-2">
            ...
            <x-card title="Timestamps">
                <dl class="space-y-1 text-xs">
                    <div>
                        ...
                        <dd class="inline">{{ $contact->created_at->format('d M Y H:i') }}</dd>
                    </div>
                    <div>
                        <dt class="inline font-medium text-zinc-500">Updated:</dt>
                        <dd class="inline">{{ $contact->updated_at->format('d M Y H:i') }}</dd>
                    </div>
                </dl>
            </x-card>
        </aside>
